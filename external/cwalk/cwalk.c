...
static size_t cwk_path_output_sized(wchar_t *buffer, size_t buffer_size,
...
static size_t cwk_path_output_current(wchar_t *buffer, size_t buffer_size,
  size_t position)
{
  // We output a "current" directory, which is a single character. This
  // character is currently not style dependant.
  return cwk_path_output_sized(buffer, buffer_size, position, L".", 1);
}

static size_t cwk_path_output_back(wchar_t *buffer, size_t buffer_size,
  size_t position)
{
  // We output a "back" directory, which ahs two characters. This
  // character is currently not style dependant.
  return cwk_path_output_sized(buffer, buffer_size, position, L"..", 2);
}

static size_t cwk_path_output_separator(wchar_t *buffer, size_t buffer_size,
  size_t position)
{
...
static size_t cwk_path_output_dot(wchar_t *buffer, size_t buffer_size,
  size_t position)
{
...
static size_t cwk_path_output(wchar_t *buffer, size_t buffer_size, size_t position,
  const wchar_t *str)
{
  size_t length;
...
static void cwk_path_terminate_output(wchar_t *buffer, size_t buffer_size,
  size_t pos)
{
  if (buffer_size > 0) {
    if (pos >= buffer_size) {
...
static bool cwk_path_is_string_equal(const wchar_t *first, const wchar_t *second,
  size_t first_size, size_t second_size)
{
  ...
  while (*first && *second && first_size > 0) {
    ...
    are_both_separators = wcschr(separators[path_style], *first) != NULL &&
        wcschr(separators[path_style], *second) != NULL;
...
static const wchar_t *cwk_path_find_next_stop(const wchar_t *c)
{
  // We just move forward until we find a '\0' or a separator, which will be our
  // next "stop".
  while (*c != L'\0' && !cwk_path_is_separator(c)) {
    ++c;
  }
...
static const wchar_t *cwk_path_find_previous_stop(const wchar_t *begin, const wchar_t *c)
{
...
static bool cwk_path_get_first_segment_without_root(const wchar_t *path,
  const wchar_t *segments, struct cwk_segment *segment)
{
...
static bool cwk_path_get_last_segment_without_root(const wchar_t *path,
  struct cwk_segment *segment)
{
...
static bool cwk_path_get_first_segment_joined(const wchar_t **paths,
...
static void cwk_path_get_root_windows(const wchar_t *path, size_t *length)
{
  const wchar_t *c;
  bool is_device_path;
  ...
  *length = 0;
  ...
  if (cwk_path_is_separator(c)) {
    ...
    // anyway.
    ++c;
    is_device_path = (*c == L'?' || *c == L'.') && cwk_path_is_separator(++c);
    if (is_device_path) {
...
static void cwk_path_get_root_unix(const wchar_t *path, size_t *length)
{
...
static bool cwk_path_is_root_absolute(const wchar_t *path, size_t length)
{
...
static void cwk_path_fix_root(wchar_t *buffer, size_t buffer_size, size_t length)
{
  size_t i;
...
static size_t cwk_path_join_and_normalize_multiple(const wchar_t **paths,
...
size_t cwk_path_get_absolute(const wchar_t *base, const wchar_t *path, wchar_t *buffer,
  size_t buffer_size)
{
  size_t i;
...
size_t cwk_path_get_relative(const wchar_t *base_directory, const wchar_t *path,
  wchar_t *buffer, size_t buffer_size)
{
...
size_t cwk_path_join(const wchar_t *path_a, const wchar_t *path_b, wchar_t *buffer,
  size_t buffer_size)
{
...
size_t cwk_path_join_multiple(const wchar_t **paths, wchar_t *buffer,
  size_t buffer_size)
{
...
void cwk_path_get_root(const wchar_t *path, size_t *length)
{
...
size_t cwk_path_change_root(const wchar_t *path, const wchar_t *new_root,
  wchar_t *buffer, size_t buffer_size)
{
  const wchar_t *tail;
...
bool cwk_path_is_absolute(const wchar_t *path)
{
  size_t length;
...
bool cwk_path_is_relative(const wchar_t *path)
{
  // The path is relative if it is not absolute.
  return !cwk_path_is_absolute(path);
}

void cwk_path_get_basename(const wchar_t *path, const wchar_t **basename,
  size_t *length)
{
  struct cwk_segment segment;
...
size_t cwk_path_change_basename(const wchar_t *path, const wchar_t *new_basename,
  wchar_t *buffer, size_t buffer_size)
{
  struct cwk_segment segment;
...
void cwk_path_get_dirname(const wchar_t *path, size_t *length)
{
  struct cwk_segment segment;
...
bool cwk_path_get_extension(const wchar_t *path, const wchar_t **extension,
  size_t *length)
{
  struct cwk_segment segment;
  const wchar_t *c;
...
bool cwk_path_has_extension(const wchar_t *path)
{
...
size_t cwk_path_change_extension(const wchar_t *path, const wchar_t *new_extension,
  wchar_t *buffer, size_t buffer_size)
{
  struct cwk_segment segment;
...
size_t cwk_path_normalize(const wchar_t *path, wchar_t *buffer, size_t buffer_size)
{
...
size_t cwk_path_get_intersection(const wchar_t *path_base, const wchar_t *path_other)
{
  bool absolute;
...
bool cwk_path_get_first_segment(const wchar_t *path, struct cwk_segment *segment)
{
  size_t length;
...
bool cwk_path_get_last_segment(const wchar_t *path, struct cwk_segment *segment)
{
...
bool cwk_path_is_separator(const wchar_t *str)
{
  const wchar_t *c;
...
size_t cwk_path_change_segment(struct cwk_segment *segment, const wchar_t *value,
  wchar_t *buffer, size_t buffer_size)
{
...
enum cwk_path_style cwk_path_guess_style(const wchar_t *path)
{
  const wchar_t *c;
  size_t root_length;
  struct cwk_segment segment;
